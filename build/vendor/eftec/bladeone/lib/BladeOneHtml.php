...
trait BladeOneHtml
{
    ...
    protected function compileEndRadio()
    {
        $r = @\array_pop($this->htmlItem);
        if (\is_null($r)) {
            return $this->showError("@EndRadio", "Missing @Radio or so many @EndRadio", \true);
        }
        return '';
    }
    protected function compileEndCheckbox()
    {
        $r = @\array_pop($this->htmlItem);
        if (\is_null($r)) {
            return $this->showError("@EndCheckbox", "Missing @Checkbox or so many @EndCheckbox", \true);
        }
        return '';
    }
    ...
    public function item($type, $valueId, $valueText, $selectedItem = '', $wrapper = '', $extra = '')
    {
        ...
        switch ($type) {
            case 'select':
                ...
                return \sprintf($wrapper, "<option {$valueHtml} {$selected} " . $this->convertArg($extra) . ">{$valueText}</option>\n");
                break;
            case 'radio':
                $selected = $found ? 'checked' : '';
                return \sprintf($wrapper, "<input type='radio' id='" . static::e($id) . "' name='" . static::e($id) . "' {$valueHtml} {$selected} " . $this->convertArg($extra) . "> {$valueText}\n");
                break;
            case 'checkbox':
                $selected = $found ? 'checked' : '';
                return \sprintf($wrapper, "<input type='checkbox' id='" . static::e($id) . "' name='" . static::e($id) . "' {$valueHtml} {$selected} " . $this->convertArg($extra) . "> {$valueText}\n");
                break;
            default:
    ...
    public function trio($type, $valueId, $valueText, $value3 = '', $selectedItem = '', $wrapper = '', $extra = '')
    {
        ...
        switch ($type) {
            case 'selectgroup':
                $selected = $found ? 'selected' : '';
                return \sprintf($wrapper, "<option value='{$valueId}' {$selected} " . $this->convertArg($extra) . ">{$valueText}</option>\n");
                break;
            default:
                return '???? type undefined: [$type] on @item<br>';
        }
    ...
    protected $paginationStructure = ['selHtml' => '<li class="selected" %3s><a href="%1s">%2s</a></li>', 'html' => '<li %3s><a href="%1s">%2s</a></li>', 'maxItem' => 5, 'url' => ''];
