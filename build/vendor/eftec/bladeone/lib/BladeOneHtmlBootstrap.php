    ...
    public function item($type, $valueId, $valueText, $selectedItem = '', $wrapper = '', $extra = '')
    {
        ...
        switch ($type) {
            ...
            case 'radio':
                ...
                return \sprintf($wrapper, "<label><input type='radio' id='" . static::e($id) . "' name='" . static::e($id) . "' {$valueHtml} {$selected} " . $this->convertArg($extra) . "> {$valueText}</label>\n");
                break;
            case 'checkbox':
                $selected = $found ? 'checked' : '';
                return \sprintf($wrapper, "<label><input type='checkbox' id='" . static::e($id) . "' name='" . static::e($id) . "' {$valueHtml} {$selected} " . $this->convertArg($extra) . "> {$valueText}</label>\n");
                break;
            default:
