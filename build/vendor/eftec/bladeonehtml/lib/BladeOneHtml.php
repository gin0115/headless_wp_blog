...
namespace PC_Headless_Blog_1AA\eftec\bladeonehtml;
...
trait BladeOneHtml
{
    ...
    public $pattern = ['input' => '{{pre}}<input{{inner}} >{{between}}</input>{{post}}', 'input_empty' => '{{pre}}<input{{inner}} />{{post}}', 'file' => '{{pre}}<input type="file"{{inner}} >{{between}}</input>{{post}}', 'select' => '{{pre}}<select{{inner}} >{{between}}{{post}}', 'select_item' => '<option{{inner}} >{{between}}</option>', 'select_end' => '</select>', 'checkbox' => '{{pre}}<input type="checkbox" {{inner}} >{{between}}</input>{{post}}', 'radio' => '{{pre}}<input type="radio" {{inner}} >{{between}}</input>{{post}}', 'textarea' => '{{pre}}<textarea {{inner}} >{{between}}</textarea>{{post}}', 'button' => '{{pre}}<button{{inner}} >{{between}}</button>{{post}}', 'link' => '{{pre}}<a{{inner}} >{{between}}</a>{{post}}', 'checkboxes' => '{{pre}}<div{{inner}} >{{between}}{{post}}', 'checkboxes_item' => '<input type="checkbox"{{inner}} >{{between}}</input>{{post}}', 'checkboxes_end' => '</div>', 'radios' => '{{pre}}<div{{inner}} >{{between}}{{post}}', 'radios_item' => '<input type="radio"{{inner}} >{{between}}</input>{{post}}', 'radios_end' => '</div>', 'ul' => '{{pre}}<ul{{inner}} >{{between}}{{post}}', 'ul_item' => '<li{{inner}} >{{between}}</li>{{post}}', 'ul_end' => '</ul>', 'ol' => '{{pre}}<ol{{inner}} >{{between}}{{post}}', 'ol_item' => '<li{{inner}} >{{between}}</li>{{post}}', 'ol_end' => '</ol>', 'optgroup' => '{{pre}}<optgroup{{inner}} >{{between}}{{post}}', 'optgroup_end' => '</optgroup>', 'table' => '{{pre}}<table{{inner}} >{{between}}{{post}}', 'table_item' => '<tr {{inner}} >{{between}}</tr>{{post}}', 'table_end' => '</table>', 'tablebody' => '<tbody {{inner}} >{{between}}{{post}}', 'tablebody_end' => '</tbody>', 'tablehead' => '<thead><tr {{inner}} >{{between}}{{post}}', 'tablehead_end' => '</tr></thead>', 'tablefooter' => '<tfoot><tr {{inner}} >{{between}}{{post}}', 'tablefooter_end' => '</tr></tfoot>', 'tablerows' => '<tr {{inner}} >{{between}}{{post}}', 'tablerows_end' => '</tr>', 'form' => '{{pre}}<form {{inner}} >{{between}}{{post}}', 'form_end' => '</form>', 'cell' => '<td {{inner}} >{{between}}</td>{{post}}', 'head' => '<th {{inner}} >{{between}}</th>{{post}}', 'label' => '{{pre}}<label {{inner}} >{{between}}</label>{{post}}', 'image' => '{{pre}}<img {{inner}} >{{between}}</img>{{post}}', 'alert' => '{{pre}}<div {{inner}}>{{between}}</div>{{post}}', 'pagination' => '{{pre}}{{between}}{{post}}'];
    ...
    public function useBootstrap3($useCDN = \false)
    {
        ...
        $bs3 = ['button' => 'btn', 'input' => 'form-control', 'textarea' => 'form-control', 'select' => 'form-control', 'file' => 'form-control-file', 'range' => 'form-control-range', 'ul' => 'list-group', 'ul_item' => 'list-group-item', 'ol' => 'list-group', 'ol_item' => 'list-group-item', 'table' => 'table', 'alert' => 'alert'];
        $this->defaultClass = \array_merge($this->defaultClass, $bs3);
        if ($useCDN) {
            $this->addCss('<link rel="stylesheet" ' . 'href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css" ' . 'integrity="sha384-BVYiiSIFeK1dGmJRAkycuHAHRg32OmUcww7on3RYdg4Va+PmSTsz/K68vbdEjh4u" ' . 'crossorigin="anonymous">', 'bootstrap');
            $this->addJs('<script src="https://code.jquery.com/jquery-3.5.0.min.js" ' . 'integrity="sha256-xNzN2a4ltkB44Mc/Jz3pT4iU1cmeR0FkXs4pru/JxaQ=" ' . 'crossorigin="anonymous"></script>', 'jquery');
            $this->addJs('<script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js" ' . 'integrity="sha384-Tc5IQib027qvyjSMfHjOMaLkfuWVxZxUPnCJA7l2mCWNIpG9mGCD8wGNIcPD7Txa" ' . 'crossorigin="anonymous"></script>', 'bootstrap');
        }
    }
    public function useBootstrap4($useCDN = \false)
    {
        $this->style = 'bootstrap4';
        $bs4 = ['button' => 'btn', 'input' => 'form-control', 'textarea' => 'form-control', 'checkbox_item' => 'form-check-input', 'select' => 'form-control', 'file' => 'form-control-file', 'range' => 'form-control-range', 'radio' => 'form-check-input', 'radio_item' => 'form-check-input', 'ul' => 'list-group', 'ul_item' => 'list-group-item', 'ol' => 'list-group', 'ol_item' => 'list-group-item', 'table' => 'table', 'alert' => 'alert'];
...
<div class="custom-control custom-radio">
     ...
     */
    protected function render($args, $pattern, $wrapper, $escapeBetween = \true)
    {
        if (isset($this->defaultClass[$pattern])) {
            $args['class'] = '"' . \trim($this->stripQuotes(@$args['class']) . ' ' . $this->defaultClass[$pattern]) . '"';
        }
        $customArgs = [];
    ...
    protected function compilePagination($expression)
    {
        if ($this->style !== 'bootstrap4' && $this->style !== 'bootstrap3') {
            ...
            use useBootstrap3() or useBootstrap4()', \true);
            return '';
        }
        $args = $this->getArgs($expression);
        if (!isset($args['numpages'], $args['current'])) {
            $this->showError('@pagination', '@pagination: Missing numpages or current arguments', \true);
            return '';
        }
        $_urlparam = isset($args['urlparam']) ? $args['urlparam'] : "'_page'";
        // if not urlparam the we use _page as default
        //unset($args['urlparam'])
        ...
        echo \'<li class="page-item"><a class="page-link" href="\'.$_url.\'" tabindex="-1">' . $this->translationControl['pagination']['first'] . '</a></li>\';
        if(' . $_current . ' >1) {
            $_url=$this->addArgUrl([' . $_urlparam . '=>' . $_current . '-1]);
            echo \'<li class="page-item"><a class="page-link" href="\'.$_url.\'" tabindex="-1">' . $this->translationControl['pagination']['prev'] . '</a></li>\';
        } else {
            echo \'<li class="page-item disabled"><a class="page-link" href="#" tabindex="-1">' . $this->translationControl['pagination']['prev'] . '</a></li>\';
        }
        ...
        if(' . $_current . ' <' . $_numpages . ') {
            $_url=$this->addArgUrl([' . $_urlparam . '=>' . $_current . '+1]);
            echo \'<li class="page-item"><a class="page-link" href="\'.$_url.\'">' . $this->translationControl['pagination']['next'] . '</a></li>\';
        } else {
            echo \'<li class="page-item disabled"><a class="page-link" href="#">' . $this->translationControl['pagination']['next'] . '</a></li>\';
        }
        $_url=$this->addArgUrl([' . $_urlparam . '=>$_p1]);
        echo \'<li class="page-item"><a class="page-link" href="\'.$_url.\'" tabindex="-1">' . $this->translationControl['pagination']['last'] . '</a></li>\';
        echo \'</ul>\';
        // pagination ends *********************************************
        ?>';
        $result = ['', $r, '', ''];
    ...
    {
        $args = $this->getArgs($expression);
        $this->htmlItem[] = ['type' => 'select', 'value' => @$args['value'], 'values' => @$args['values'], 'alias' => @$args['alias'], 'id' => @$args['id'], 'name' => null, 'idname' => null];
        $result = ['', '', '', ''];
        // inner, between, pre, post
    ...
    protected function compileEndSelect()
    {
        ...
        if ($parent === null) {
            $this->showError('@endselect', 'Missing @select or so many @endselect', \true);
        }
    ...
    protected function compileItems($expression)
    {
        ...
        if ($args['value'] === null) {
            $this->showError('@items with missing tag value', '@items' . $expression, \true);
        }
        if ($args['values'] === null) {
            $this->showError('@items with missing tag values', '@items' . $expression, \true);
        }
        if ($args['alias'] === null) {
            if ($this->isVariablePHP($args['values'])) {
                $args['alias'] = $args['values'] . 'Row';
            } else {
                $this->showError('@items with missing tag alias', '@items' . $expression, \true);
            }
        }
        $result = ['', '', '', ''];
        // inner, between, pre, post
        ...
        $html = '<?php ' . $nameOG . '=\'\';  foreach(' . $name . ' as ' . $nameKey . '=>' . $args['alias'] . ') {' . "\n";
    ...
    {
        $args = $this->getArgs($expression);
        $this->htmlItem[] = ['type' => 'checkboxes', 'value' => @$args['value'], 'values' => @$args['values'], 'alias' => @$args['alias'], 'id' => @$args['id'], 'name' => @$args['name'], 'idname' => @$args['idname']];
        unset($args['values'], $args['alias']);
        $result = ['', '', '', ''];
        // inner, between, pre, post
    ...
    protected function compileEndCheckboxes()
    {
        $parent = @\array_pop($this->htmlItem);
        if ($parent === null) {
            $this->showError('@endcheckboxes', 'Missing @checkboxes or so many @checkboxes', \true);
        }
        return $this->pattern[$parent['type'] . '_end'];
    }
    ...
    {
        $args = $this->getArgs($expression);
        $this->htmlItem[] = ['type' => 'radios', 'value' => @$args['value'], 'values' => @$args['values'], 'alias' => @$args['alias'], 'id' => @$args['id'], 'name' => @$args['name'], 'idname' => @$args['idname']];
        unset($args['values'], $args['alias']);
        $result = ['', '', '', ''];
        // inner, between, pre, post
    ...
    protected function compileEndRadios()
    {
        $parent = @\array_pop($this->htmlItem);
        if ($parent === null) {
            $this->showError('@endradios', 'Missing @radios or so many @radios', \true);
        }
        return $this->pattern[$parent['type'] . '_end'];
    }
    ...
    {
        $args = $this->getArgs($expression);
        $this->htmlItem[] = ['type' => 'ul', 'value' => @$args['value'], 'values' => @$args['values'], 'alias' => @$args['alias'], 'id' => @$args['id'], 'name' => @$args['name'], 'idname' => @$args['idname']];
        unset($args['values'], $args['alias']);
        $result = ['', '', '', ''];
        // inner, between, pre, post
    ...
    protected function compileEndUl()
    {
        $parent = @\array_pop($this->htmlItem);
        if ($parent === null) {
            $this->showError('@endul', 'Missing @ul or so many @endul', \true);
        }
        return $this->pattern[$parent['type'] . '_end'];
    }
    ...
    {
        $args = $this->getArgs($expression);
        $this->htmlItem[] = ['type' => 'ol', 'value' => @$args['value'], 'values' => @$args['values'], 'alias' => @$args['alias'], 'id' => @$args['id'], 'name' => @$args['name'], 'idname' => @$args['idname']];
        unset($args['values'], $args['alias']);
        $result = ['', '', '', ''];
        // inner, between, pre, post
    ...
    protected function compileEndOl()
    {
        $parent = @\array_pop($this->htmlItem);
        if ($parent === null) {
            $this->showError('@endol', 'Missing @ol or so many @endol', \true);
        }
        return $this->pattern[$parent['type'] . '_end'];
    }
    ...
    {
        if (!$this->insideForm) {
            $this->showError('@endform', 'Missing @form or so many @endform', \true);
        }
    ...
    {
        $args = $this->getArgs($expression);
        $this->htmlItem[] = ['type' => 'table', 'value' => @$args['values'], 'id' => null, 'name' => null, 'idname' => null, 'alias' => @$args['alias']];
        unset($args['values'], $args['alias']);
        $result = ['', '', '', ''];
        // inner, between, pre, post
    ...
    protected function compileEndTableBody($expression)
    {
        $parent = @\array_pop($this->htmlItem);
        if ($parent === null) {
            $this->showError('@endtablebody', 'Missing @tablebody or so many @endtablebody', \true);
        }
        $args = $this->getArgs($expression);
        $result = ['', '', '', ''];
        // inner, between, pre, post
    ...
    protected function compileEndTableHead($expression)
    {
        $parent = @\array_pop($this->htmlItem);
        if ($parent === null) {
            $this->showError('@endtablehead', 'Missing @tablehead or so many @endtablehead', \true);
        }
        $args = $this->getArgs($expression);
        $result = ['', '', '', ''];
        // inner, between, pre, post
    ...
    protected function compileEndTableRows($expression)
    {
        $parent = @\array_pop($this->htmlItem);
        if ($parent === null) {
            $this->showError('@endtablerows', 'Missing @tablerows or so many @endtablerows', \true);
        }
        $args = $this->getArgs($expression);
        $result = ['', '', '', ''];
        // inner, between, pre, post
