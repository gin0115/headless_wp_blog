 ...
 * @version 3.0 */
namespace PC_Headless_Blog_1AA\Dice\Loader;

class Xml
{
    private function getComponent(\PC_Headless_Blog_1AA\SimpleXmlElement $element, $forceInstance = \false)
    {
        if ($forceInstance) {
            return [\PC_Headless_Blog_1AA\Dice\Dice::INSTANCE => (string) $element];
        } else {
            if ($element->instance) {
                return [\PC_Headless_Blog_1AA\Dice\Dice::INSTANCE => (string) $element->instance];
            } else {
                return (string) $element;
            }
        }
    }
    private function loadV1(\PC_Headless_Blog_1AA\SimpleXmlElement $xml, \PC_Headless_Blog_1AA\Dice\Dice $dice)
    {
        $rules = [];
        foreach ($xml as $key => $value) {
            ...
            if (isset($value->inherit)) {
                $rule['inherit'] = $value->inherit == 'false' ? \false : \true;
            }
            if ($value->call) {
            ...
            if ($value->substitutions) {
                foreach ($value->substitutions as $use) {
                    $rule['substitutions'][(string) $use->as] = $this->getComponent($use->use, \true);
                }
            }
    ...
    private function loadV2(\PC_Headless_Blog_1AA\SimpleXmlElement $xml, \PC_Headless_Blog_1AA\Dice\Dice $dice)
    {
        $rules = [];
        foreach ($xml as $key => $value) {
            $rule = $dice->getRule((string) $value->name);
    ...
    public function load($xml, \PC_Headless_Blog_1AA\Dice\Dice $dice = null, $displayWarning = \true)
    {
        if ($displayWarning) {
        ...
        if ($dice === null) {
            $dice = new \PC_Headless_Blog_1AA\Dice\Dice();
        }
        if (!$xml instanceof \PC_Headless_Blog_1AA\SimpleXmlElement) {
