...
trait MessageTrait
{
    ...
    public function getBody() : \PC_Headless_Blog_1AA\Psr\Http\Message\StreamInterface
    {
        if (null === $this->stream) {
            $this->stream = \PC_Headless_Blog_1AA\Nyholm\Psr7\Stream::create('');
        }
        return $this->stream;
    }
    public function withBody(\PC_Headless_Blog_1AA\Psr\Http\Message\StreamInterface $body) : self
    {
        if ($body === $this->stream) {
            return $this;
        }
    ...
    private function validateAndTrimHeader($header, $values) : array
    {
        ...
        if (!\is_array($values)) {
            ...
            if (!\is_numeric($values) && !\is_string($values) || 1 !== \preg_match("@^[ \t!-~€-ÿ]*\$@", (string) $values)) {
        ...
        $returnValues = [];
        foreach ($values as $v) {
            if (!\is_numeric($v) && !\is_string($v) || 1 !== \preg_match("@^[ \t!-~€-ÿ]*\$@", (string) $v)) {
                throw new \InvalidArgumentException('Header values must be RFC 7230 compatible strings.');
            }
