...
class Hook_Manager_Object_Mock extends \PC_Headless_Blog_1AA\PinkCrab\Loader\Hook_Manager
{
    ...
    {
        $this->_hooks['actions'][$handle] = array('handle' => $handle, 'callback' => $callback, 'priority' => $priority, 'args' => $args);
    }
    /** Adds and filter to the internal set. */
    public function _add_filter($handle, $callback, $priority = 10, $args = 1)
    {
        $this->_hooks['filters'][$handle] = array('handle' => $handle, 'callback' => $callback, 'priority' => $priority, 'args' => $args);
    }
    /** Adds a remove hook record to the internal set. */
    public function _remove_hook($handle, $callback, $priority = 10)
    {
        $this->_hooks['remove'][$handle] = array('handle' => $handle, 'callback' => $callback, 'priority' => $priority);
    }
    /** Clears the internal state. */
    public function _clear() : void
    {
        $this->_hooks = array('actions' => array(), 'filters' => array(), 'remove' => array());
    }
    /**
