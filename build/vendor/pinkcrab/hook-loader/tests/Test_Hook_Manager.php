...
use PC_Headless_Blog_1AA\Automattic\Jetpack\Constants;
...
class Test_Hook_Manager extends \WP_UnitTestCase
{
    ...
    public function test_hooks_are_processed_by_correct_register_method()
    {
        ...
        $action = (new \PC_Headless_Blog_1AA\PinkCrab\Loader\Hook('action', 'is_string'))->type(\PC_Headless_Blog_1AA\PinkCrab\Loader\Hook::ACTION);
        $filter = (new \PC_Headless_Blog_1AA\PinkCrab\Loader\Hook('filter', 'is_bool'))->type(\PC_Headless_Blog_1AA\PinkCrab\Loader\Hook::FILTER);
        $remove = (new \PC_Headless_Blog_1AA\PinkCrab\Loader\Hook('remove', 'is_int'))->type(\PC_Headless_Blog_1AA\PinkCrab\Loader\Hook::REMOVE);
        $manager = new \PC_Headless_Blog_1AA\PinkCrab\Loader\Tests\Fixtures\Hook_Manager_Object_Mock();
        // Process them.
    ...
    {
        $invalid = (new \PC_Headless_Blog_1AA\PinkCrab\Loader\Hook('remove', 'is_int'))->type('INVALID');
        $manager = new \PC_Headless_Blog_1AA\PinkCrab\Loader\Tests\Fixtures\Hook_Manager_Object_Mock();
        $invalid = $manager->process_hook($invalid);
        $this->assertFalse($invalid->is_registered());
    }
    public function FunctionName(\PC_Headless_Blog_1AA\PinkCrab\Loader\Tests\Type $var = null)
    {
        # code...
    }
}
