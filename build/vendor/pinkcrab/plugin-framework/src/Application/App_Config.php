...
final class App_Config
{
    ...
    {
        return \array_key_exists($key, $this->namespaces) ? $this->namespaces[$key] : null;
    }
    /**
    ...
    {
        return \array_key_exists($key, $this->additional) ? $this->additional[$key] : null;
    }
    /**
    ...
    public function post_types(string $key, string $field = 'slug', ?string $meta_key = null)
    {
        ...
        return $meta_key ? $this->post_types[$key]['meta'][$meta_key] : $this->post_types[$key]['meta'];
    }
    /**
    ...
    protected function set_post_types(array $post_types) : void
    {
        foreach ($post_types as $label => $post_type) {
            ...
            $this->post_types[$label] = array('slug' => $post_type['slug'], 'meta' => $post_type['meta']);
        }
    }
    /**
     * Returns the key for a post type.
    ...
    public function taxonomies(string $key, string $field = 'slug', ?string $term_key = null)
    {
        ...
        return $term_key ? $this->taxonomies[$key]['term'][$term_key] : $this->taxonomies[$key]['term'];
    }
    /**
    ...
    protected function set_taxonomies(array $taxonomies) : void
    {
        foreach ($taxonomies as $label => $taxonomy) {
            ...
            $this->taxonomies[$label] = array('slug' => $taxonomy['slug'], 'term' => $taxonomy['term']);
        }
    }
    /**
