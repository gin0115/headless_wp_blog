...
use PC_Headless_Blog_1AA\PinkCrab\Registerables\Tests\Fixtures\Ajax\Ajax_With_Scripts;
class Test_Ajax_With_Scripts extends \WP_UnitTestCase
{
    protected function server_request_provider() : \PC_Headless_Blog_1AA\Nyholm\Psr7\ServerRequest
    {
        return (new \PC_Headless_Blog_1AA\PinkCrab\HTTP\HTTP())->request_from_globals();
    }
    /**
    ...
    {
        $ajax_instance = new \PC_Headless_Blog_1AA\PinkCrab\Registerables\Tests\Fixtures\Ajax\Ajax_With_Scripts($this->server_request_provider());
        // Just run setup to set the scripts.
        $ajax_instance->set_up();
        // Extract scripts as an array.
        $scripts = \PC_Headless_Blog_1AA\Gin0115\WPUnit_Helpers\Objects::get_property($ajax_instance, 'scripts')->to_array();
        $this->assertInstanceOf(\PC_Headless_Blog_1AA\PinkCrab\Enqueue\Enqueue::class, $scripts[0]);
        $this->assertEquals('ajax_with_scripts_one', \PC_Headless_Blog_1AA\Gin0115\WPUnit_Helpers\Objects::get_property($scripts[0], 'handle'));
        $this->assertEquals('script', \PC_Headless_Blog_1AA\Gin0115\WPUnit_Helpers\Objects::get_property($scripts[0], 'type'));
        $this->assertEquals('http://www.url.tld/file.js', \PC_Headless_Blog_1AA\Gin0115\WPUnit_Helpers\Objects::get_property($scripts[0], 'src'));
        $this->assertInstanceOf(\PC_Headless_Blog_1AA\PinkCrab\Enqueue\Enqueue::class, $scripts[1]);
        $this->assertEquals('ajax_with_scripts_two', \PC_Headless_Blog_1AA\Gin0115\WPUnit_Helpers\Objects::get_property($scripts[1], 'handle'));
        $this->assertEquals('script', \PC_Headless_Blog_1AA\Gin0115\WPUnit_Helpers\Objects::get_property($scripts[1], 'type'));
        $this->assertGreaterThan(0, \strpos(\PC_Headless_Blog_1AA\Gin0115\WPUnit_Helpers\Objects::get_property($scripts[1], 'src'), 'Ajax/file.js'));
    }
    /**
    ...
    public function test_scripts_added_to_loader_front_end() : void
    {
        ...
        $scripts = \PC_Headless_Blog_1AA\Gin0115\WPUnit_Helpers\Objects::get_property($loader, 'front');
        $scripts = \PC_Headless_Blog_1AA\Gin0115\WPUnit_Helpers\Objects::get_property($scripts, 'hooks');
    ...
    public function test_loader_enqueues_scripts() : void
    {
        ...
        $registered_scripts = \PC_Headless_Blog_1AA\Gin0115\WPUnit_Helpers\Objects::get_property($GLOBALS['wp_scripts'], 'registered');
        // Check first script.
    ...
    public function text_optional_conditional_can_be_used() : void
    {
        ...
        // Set to true.
        $ajax_instance->_conditional_value = \true;
