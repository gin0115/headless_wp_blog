...
class Base_CPT_Case extends \WP_UnitTestCase
{
    ...
    {
        $this->posts = \array_map(function ($e) {
            return get_post($e);
        }, array($this->factory->post->create(array('post_type' => $this->cpt->key)), $this->factory->post->create(array('post_type' => $this->cpt->key)), $this->factory->post->create(array('post_type' => $this->cpt->key)), $this->factory->post->create(array('post_type' => $this->cpt->key)), $this->factory->post->create(array('post_type' => $this->cpt->key))));
        $this->posts;
    }
    /**
    ...
    public function test_permalink_uses_key_for_slug() : void
    {
        if ($this->has_single) {
            ...
            $this->assertTrue(\true);
            // Well it nots giving me a warning ;)
        }
    }
    /**
    ...
    public function test_post_type_supports() : void
    {
        foreach ($this->supports as $feature => $expected) {
            $this->assertEquals($expected, post_type_supports($this->cpt->key, $feature), \sprintf('Failed asserting that %s was %s for %s', $feature, $expected ? 'TRUE' : 'FALSE', $this->cpt->key));
        }
    }
    /**
    ...
    public function test_post_type_settings()
    {
        foreach ($this->settings as $property => $expected) {
            $this->assertEquals($expected, get_post_type_object($this->cpt->key)->{$property}, \sprintf('Failed asserting setting that %s was %s for %s', $property, $expected ? 'TRUE' : 'FALSE', $this->cpt->key));
        }
    }
    /**
    ...
    public function test_user_role_access_create_posts() : void
    {
        foreach ($this->user_access_create as $role => $expected) {
            ...
            $this->assertEquals($expected, current_user_can(get_post_type_object($this->cpt->key)->cap->create_posts), \sprintf('Failed asserting that role %s could create post. Expected %s for %s', $role, $expected ? 'TRUE' : 'FALSE', $this->cpt->key));
        }
    }
    /**
    ...
    public function test_user_role_access_view_posts() : void
    {
        foreach ($this->user_access_view as $role => $expected) {
            wp_set_current_user($this->factory->user->create(array('role' => $role)));
            $this->assertEquals($expected, current_user_can(get_post_type_object($this->cpt->key)->cap->read), \sprintf('Failed asserting that role %s could view post. Expected %s for %s', $role, $expected ? 'TRUE' : 'FALSE', $this->cpt->key));
        }
    }
    /**
    ...
    public function test_user_role_access_delete_posts() : void
    {
        foreach ($this->user_access_delete as $role => $expected) {
            wp_set_current_user($this->factory->user->create(array('role' => $role)));
            $this->assertEquals($expected, current_user_can(get_post_type_object($this->cpt->key)->cap->delete_posts), \sprintf('Failed asserting that role %s could delete post. Expected %s for %s', $role, $expected ? 'TRUE' : 'FALSE', $this->cpt->key));
        }
    }
    /**
     * Test that defined user role access is allowed to delete posts.
    ...
    public function test_user_role_access_edit_other_users_posts() : void
    {
        foreach ($this->user_access_edit_others as $role => $expected) {
            wp_set_current_user($this->factory->user->create(array('role' => $role)));
            $this->assertEquals($expected, current_user_can(get_post_type_object($this->cpt->key)->cap->edit_others_posts), \sprintf('Failed asserting that role %s could edit other users post. Expected %s for %s', $role, $expected ? 'TRUE' : 'FALSE', $this->cpt->key));
        }
    }
}
