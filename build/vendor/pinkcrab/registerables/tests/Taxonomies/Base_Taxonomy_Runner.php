...
class Base_Taxonomy_Runner extends \WP_UnitTestCase
{
    ...
    {
        for ($i = 0; $i < 5; $i++) {
            $this->terms[] = $this->factory->term->create(array('taxonomy' => $this->taxonomy->slug));
        }
    }
    /**
     * Test the labels.
    ...
    {
        $registered_post_types = \array_key_exists($this->taxonomy->slug, $this->wp_taxonomies) ? $this->wp_taxonomies[$this->taxonomy->slug]->object_type : array();
    ...
    public function test_settings() : void
    {
        ...
        foreach ($this->settings as $property => $expected) {
            ...
            if ($property === 'rewrite') {
                $this->assertEquals($expected, $wp_taxonomy->{$property}['slug'], \sprintf('Failed asserting setting that %s was %s for %s (rewrite, so looking at slug property', $property, $expected ? 'TRUE' : 'FALSE', $this->taxonomy->slug));
            } else {
                // Match all other proprerties.
                $this->assertEquals($expected, $wp_taxonomy->{$property}, \sprintf('Failed asserting setting that %s was %s for %s', $property, $expected ? 'TRUE' : 'FALSE', $this->taxonomy->slug));
            }
        }
    }
}
